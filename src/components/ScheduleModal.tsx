      ...
      <ModalContainer onClose={onClose} backdropClassName="bg-black/80 backdrop-blur-sm">
        <div className="w-full h-full max-w-[95vw] max-h-[95vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
          ...
          <div className="flex-shrink-0 px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Course Schedule</h2>
              <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">{semester.name}</p>
            </div>
            <div className="flex items-center gap-3">
              {scheduleBlocks.length > 0 && (
                <button
                  ...
                  className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-indigo-500 to-purple-500 text-white font-bold rounded-xl hover:scale-105 transition-transform shadow-lg"
                >
            ...
            {scheduleBlocks.length === 0 && !showAddForm ? (
              ...
              <div className="relative">
                ...
                <div className="grid grid-cols-8 gap-4 min-w-[1200px]">
                  {/* Time column */}
                  <div className="sticky left-0 z-10 bg-white dark:bg-gray-900 w-24" style={{ boxShadow: '2px 0 4px -1px rgba(0, 0, 0, 0.1)', marginLeft: '-1.5rem', paddingLeft: '1.5rem' }}>
                    <div className="h-12 bg-white dark:bg-gray-900"></div>
                    {TIME_SLOTS.map((hour, idx) => (
                      <div
                        key={idx}
                        className="h-8 border-r pr-2 text-xs text-gray-500 dark:text-gray-400 text-right"
                          ...
                          return (
                            <div
                              ...
                              className="absolute left-1 right-1 rounded-lg p-2.5 cursor-pointer hover:opacity-90 transition-opacity shadow-sm border-l-4 group"
                                ...
                                top: `${top}%`,
                                height: `${height}%`,
                                backgroundColor: `${getCourseColor(block.courseId)}40`, // Increased opacity for better visibility in light mode
                                borderLeftColor: getCourseColor(block.courseId),
                                borderLeftWidth: '4px',
                                minHeight: '32px',
                                zIndex: 10, // Higher z-index to ensure blocks are above grid lines
